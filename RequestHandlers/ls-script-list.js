...
const handleRequest = (params) => {
    console.log(`${new Date().toISOString()} ${params.uuid} ls-script-list executing.`);
    return new Promise((resolve, reject) => {
        ...
        buildResponse((err, listObj) => {
            if (err) {
                ...
                var responseObj = new responseBuilder.ResponseObj(
                    'merlin-response', 
                    {'request-type': params.requestType, 'response-type': params.requestType, 'user-name': require('uuid/v1')().substring(0,7), 'last-modified': `${new Date().toISOString()}`, 'script-count': listObj.length}
                );
